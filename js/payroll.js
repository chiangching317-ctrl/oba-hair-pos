...
function buildAuthorityPayrollPageModel(result,revision){
  ...
  const month=payrollTrialMonth(result?.month||overview.month);
  const localSalaryOrders=typeof salaryBaseOrders==='function'?salaryBaseOrders().filter(order=>String(order?.date||'').startsWith(month)&&!order?.refunded):[];
  const redeemSummaryForStaff=staffId=>{
    const rows=localSalaryOrders.filter(order=>String(order.assignedDesignerId||'')===String(staffId)&&typeof isRedeemOrder==='function'&&isRedeemOrder(order));
    return {
      redeemCount:rows.length,
      redeemSourceTotal:rows.reduce((sum,order)=>sum+(typeof redemptionSourceValue==='function'?redemptionSourceValue(order):Number(order?.redeemMeta?.sourcePrice||0)),0),
      redeemCommissionTotal:rows.reduce((sum,order)=>sum+Number(order.commission||0),0)
    };
  };
...
function buildDraftPayrollPageModel(monthValue,revision){
  ...
  const employees=summary.trials.map(row=>{const isJean=String(row.staff.name||row.staff.nickname||row.staff.id||'').trim().toUpperCase()==='JEAN';const actualSalary=payrollModelNumber(row.actualSalary);return {staffId:String(row.staff.id||''),staffName:String(row.staff.name||row.staff.nickname||row.staff.id||''),orderCount:payrollModelNumber(row.orderCount),performanceTotal:payrollModelNumber(row.performanceTotal),commissionTotal:payrollModelNumber(row.commissionTotal),redeemCount:payrollModelNumber(row.redeemCount),redeemSourceTotal:payrollModelNumber(row.redeemSourceTotal),redeemCommissionTotal:payrollModelNumber(row.redeemCommissionTotal),guaranteeSalary:PAYROLL_GUARANTEE,responsibilityTarget:PAYROLL_TARGET,achievementRate:Number(row.achievementRate||0),baseSalary:payrollModelNumber(row.baseSalary),advance:payrollModelNumber(row.advance),materialAdvance:payrollModelNumber(row.materialAdvance),insuranceSelf:payrollModelNumber(row.insurance),annualBonus:payrollModelNumber(row.annualBonus),actualSalary,managementShare:isJean?managementShare:0,monthlyTotal:actualSalary+(isJean?managementShare:0)};});
