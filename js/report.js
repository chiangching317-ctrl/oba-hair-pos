...
function reportOrderInSalaryPeriod(order){
  return reportOrderAffectsTotals(order) && !!order?.assignedDesignerId && isAfterStamp(timeStampOfOrder(order),reportSalaryResetStamp());
}
function buildReportExportData(filters=reportFilterValues(),ctx=reportViewerContext()){
  ...
  scopedOrders.forEach(order=>{
    ...
    const affectsTotals=reportOrderAffectsTotals(order);
    const performanceTotal=affectsTotals?Number(orderPerformanceSnapshot(order)||0):0;
    const savedCommission=affectsTotals?audit.savedTotal:0;
    ...
    orderRows.push({
      ...
      '單據總抽成':savedCommission,'本期計入抽成':common['本期計入單據抽成'],
...
function showStaffSalesDetail(staffId, month){
  ...
  list.forEach(order=>{
    ...
    const source=reportOrderSource(order),redeem=isRedeemOrder(order),displayPerformance=redeem?0:Number(orderPerformanceSnapshot(order)||0);
    audit.rows.forEach(row=>rows.push(`<tr><td>${reportEscapeHtml((order.date||'')+' '+(order.time||''))}</td><td>${reportEscapeHtml(order.id||'')}</td><td>${reportEscapeHtml((redeem?'集點卡兌換｜':'')+(row.item?.name||''))}</td><td>${reportEscapeHtml(source)}</td><td>${money(row.item?.sourcePrice||row.item?.originalPrice||row.item?.price||0)}</td><td>${money(row.base||0)}</td><td>${order.assignedDesignerId?(Number(row.rate||0)*100).toFixed(2).replace(/\.00$/,'')+'%':'—'}</td><td>${money(row.commission||0)}</td><td>${money(displayPerformance)}</td><td>${money(audit.savedTotal)}</td><td class="${audit.consistent?'':'audit-warning'}">${reportEscapeHtml(reportOrderStatus(order))}<br>${reportEscapeHtml(audit.message)}</td></tr>`));
    if(audit.difference!==0)rows.push(`<tr><td>${reportEscapeHtml((order.date||'')+' '+(order.time||''))}</td><td>${reportEscapeHtml(order.id||'')}</td><td class="audit-warning">訂單抽成快照差額</td><td>${reportEscapeHtml(source)}</td><td>$0</td><td>$0</td><td>—</td><td class="audit-warning">${money(audit.difference)}</td><td>${money(displayPerformance)}</td><td>${money(audit.savedTotal)}</td><td class="audit-warning">歷史未保存逐項快照</td></tr>`);
  });
...
function renderReport(){
  ...
  const allRangeRecordOrders=allReportOrders.filter(o=>isDateTimeInReportRange(o.date,o.time,f));
  const allRangeOrders=allRangeRecordOrders.filter(reportOrderAffectsTotals);
  ...
  const rangeRecordOrders=ctx.canViewAll ? allRangeRecordOrders : allRangeRecordOrders.filter(reportOrderVisibleToViewer);
  ...
  if(monthlyBox){
    ...
    monthlyBox.innerHTML=payrollStaff.length
      ? payrollStaff.map(s=>{
          ...
          const redeemList=list.filter(isRedeemOrder),redeemValue=redeemList.reduce((sum,o)=>sum+redemptionSourceValue(o),0),redeemCommission=redeemList.reduce((sum,o)=>sum+Number(o.commission||0),0);
          const redeemRates=Array.from(new Set(redeemList.flatMap(o=>reportCommissionAudit(o).rows.map(row=>(Number(row.rate||0)*100).toFixed(2).replace(/\.00$/,'')+'%')))).join('、')||'—';
          const redeemSummary=redeemList.length?`<div class="report-redeem-summary"><strong>集點卡兌換</strong><br>業績 ${money(0)}｜原服務價值／計薪原價 ${money(redeemValue)}<br>抽成％ ${redeemRates}｜抽成金額 ${money(redeemCommission)}</div>`:'';
          return `<div class="stat"><div class="k">${s.name} ${f.startDate} 小計</div><div class="v">${money(daySales)} / ${money(dayCommission)} / ${dayList.length}</div><div class="space"></div><div class="k">本月業績 / 抽成 / 筆數</div><div class="v">${money(sales)} / ${money(commission)} / ${list.length}</div>${redeemSummary}<div class="space"></div><button class="btn btn-soft" type="button" onclick="showStaffSalesDetail('${String(s.id).replace(/'/g,'\'')}', '${month}')">查看明細</button></div>`;
        }).join('')
