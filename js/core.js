...
const OBA_RUNTIME_STORAGE_KEYS=window.OBA_RUNTIME_CONFIG?.storageKeys||{};
const KEY=String(OBA_RUNTIME_STORAGE_KEYS.state||'');
const RESET_MARKER_KEY=String(OBA_RUNTIME_STORAGE_KEYS.resetMarker||'');
if(!KEY||!RESET_MARKER_KEY)throw new Error('OBA_RUNTIME_STORAGE_CONFIG_INVALID');
const EXPENSE_KEY='oba_hair_dev_expenses_v11101'; // V11.1.04a：沿用 V11.1.01~V11.1.03 支出本機資料，不寫入正式雲端
// Public source contains no staff, service-price or business-data fallback.
// A valid server entry session must load the real DEV state before rendering.
const defaultState={
 monthlyOrderCounter:{},cart:[],pendingPay:'',pendingCheckoutCart:null,
 staff:[],items:[],orders:[],refunds:[],assignLogs:[],timeClockLogs:[]
};
const MASTER_PIN_6=''; // V11.1.17：安全止血，移除前端預設萬用 PIN
// Never hydrate business state before secure entry authentication. Cloud remains
// authoritative; the pre-existing cache is left untouched until an authenticated
// pull succeeds and replaces it with credential-stripped cloud state.
let state=ensureBranchFields(clone(defaultState)), pendingLockedTab='', pwdMode='enter', CURRENT_CASHIER=null, CURRENT_LOGIN_LEVEL='staff';

// V11.1.17 安全止血：取消舊版第一次開啟時強制把管理密碼與員工 PIN 重設成固定值。
const PIN_MIGRATE_KEY=String(OBA_RUNTIME_STORAGE_KEYS.pinMigration||'');
if(!PIN_MIGRATE_KEY)throw new Error('OBA_RUNTIME_STORAGE_CONFIG_INVALID');
...
function loadState(){try{const raw=localStorage.getItem(KEY);if(!raw)return ensureBranchFields(clone(defaultState));const data=ensureBranchFields(Object.assign(clone(defaultState),JSON.parse(raw)));if(data.staff.length&&!data.staff.find(s=>s.owner))data.staff[0].owner=true;return data}catch(e){return ensureBranchFields(clone(defaultState))}}
